...
root.addView(
    ...
    private fun showHome() {
        ...
        addModuleGrid(
            ...
            when (index) {
                ...
                5 -> showInstagram()
                6 -> showAnalytics()
                7 -> setCommandAndExecute(
                    "Show marketing analytics"
                )
                8 -> setCommandAndExecute(
                    "Build a test app"
                )
                9 -> setCommandAndExecute(
                    "Create an automation workflow"
                )
                10 -> setCommandAndExecute(
                    "Create a creative concept"
                )
                11 -> setCommandAndExecute(
                    "Build a test app"
                )
            }
        }
    ...
    private fun addCommandBox() {
        ...
        val input = EditText(this).apply {
            hint =
                "e.g. Create a YouTube video on AI"
            setHintTextColor(
                Color.parseColor("#627895")
            )
            setTextColor(Color.WHITE)
            textSize = 12f
            setSingleLine(false)
